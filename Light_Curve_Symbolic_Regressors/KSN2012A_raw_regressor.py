...
df = pd.read_csv('/Users/mark/Documents/Summer UROP 2025/One_Rising_Component_Curves/KSN2012A_raw_data.csv', sep=r'\s+')
df.columns = df.columns.str.strip()

t_exp = df[df["KJD-T_EXP"] > 0]["KJD"].min()
t_max = df[df["KJD-T_MAX"] > 0]["KJD"].min()
margin = 5  # You can adjust this value
...
y_eval = y[eval_mask]
y_pred_eval = y_pred_train[eval_mask]
...
x_plot = x_data[eval_mask]
y_plot = y_data[eval_mask]
y_error_plot = y_error[eval_mask]

# Plot the observed data and best-fit curve
plt.figure(figsize=(8, 5))
plt.errorbar(x_plot, y_plot, yerr=y_error_plot, fmt='o', color='black', alpha=0.1, label="Observed Data", capsize=3)
