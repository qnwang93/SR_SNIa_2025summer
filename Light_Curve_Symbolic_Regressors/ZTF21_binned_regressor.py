...
df = pd.read_csv('/Users/mark/Documents/Summer UROP 2025/Two_Rising_Component_Curves/ZTF21_binned_data.csv', delim_whitespace=True)

df['time'] = df['MJD'] - df['MJD'].min()

x_temp = df['time'].values
y_temp = df['norm_flux'].values

T_exp = None
for i in range(2, len(y_temp)):
    flux_subset = y_temp[:i+1]
    mean_flux = np.mean(y_temp[:2])
    std_flux = np.std(flux_subset)
    if y_temp[i] > mean_flux + 1 * std_flux:
        T_exp = x_temp[i]
        break
...
y_fit = df_fit['norm_flux'].values
y_fit_error = df_fit['norm_flux_err'].values

df_plot = df[(df['time'] >= T_exp) & (df['time'] <= T_max)]
x_data = df_plot['time'].values
y_data = df_plot['norm_flux'].values
y_error = df_plot['norm_flux_err'].values
y_noisy = y_data
