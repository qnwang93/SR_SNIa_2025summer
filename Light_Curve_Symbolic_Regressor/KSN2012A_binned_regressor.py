...
df = pd.read_csv("/Users/mark/Documents/Summer UROP 2025/One_Rising_Component_Curves/KSN2012A_binned_data.csv",sep=",")
df.columns = df.columns.str.strip()

t_exp = df[df["KJD - T_EXP"] > 0]["KJD"].min()
t_max = df[df["KJD - T_MAX"] > 0]["KJD"].min()
margin = 5  # Adjustable margin in same units as KJD
...
plt.title("PySR Symbolic Regression Fit to KSN2012A Light Curve")
plt.xlabel("Time")
plt.ylabel("Flux")
plt.legend()
plt.grid(True)
